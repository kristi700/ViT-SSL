...
class SimMIMViT(nn.Module):
    def __init__(
        ...
        self.mask_token = nn.Parameter(torch.randn(1, 1, encoder_embed_dim))
        self.positional_embedding = nn.Parameter(
            torch.rand(1, (input_shape[1] // patch_size) ** 2, encoder_embed_dim)
        )
        self.simmim_head = nn.Linear(
            encoder_embed_dim, input_shape[0] * patch_size * patch_size
        )
